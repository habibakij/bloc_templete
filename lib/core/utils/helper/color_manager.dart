import 'package:flutter/material.dart';

class AppColors {
  static const toneColor = Color(0XFFc2a756);
  static const primaryColor = Color(0XFF2596be);
  static const primaryLiteColor = Color(0XFF55b9dd);
  static const primaryDarkColor = Color(0XFF2286aa);
  static const primaryDarkTinColor = Color(0XFF0d3240);
  static const white = Color(0XFFffffff);
  static const whiteTextColor = Color(0XFFf2f2f2);
  static const black = Color(0XFF000000);
  static const blackTin = Color(0XFF0d0d0d);
  static const red = Color(0XFFff1100);
  static const darkRed = Color(0XFFb32400);
  static const grey = Color(0XFFaaaaaa);
  static const greyLite = Color(0XFF999999);
  static const greyLiteBorder = Color(0XFFd9d9d9);
  static const textColor = Color(0XDD000000);
  static const orangeColor = Color(0XFFff9900);
  static const green = Color(0XFF009933);
  static const greenLite = Color(0XFF99ffbb);
  static const yellow = Color(0XFFF5A905);
  static const warning = Color(0XFFffc34d);
}
